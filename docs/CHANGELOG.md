...
- [新功能] 新增可手动触发的「定制执行任务（DeepSeek）」Actions 工作流，支持基于 `OPENAI_API_KEY` 默认调用 `deepseek-v4-pro` 并自定义运行模式/股票列表。
- [文档] 更新中英文部署文档，补充「定制执行任务（DeepSeek）」所需 Secrets/Variables 配置与手动触发步骤。
